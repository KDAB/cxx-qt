...
impl CxxQtBuilder {
    ...
    fn build_object_file(builder: &cc::Build, file_path: impl AsRef<Path>, object_path: PathBuf) {
        ...
        if dir::is_exporting() {
            ...
            obj_builder
                ...
                .compile(file_path.as_ref().file_stem().unwrap().to_str().unwrap());
        }
    }
    ...
    fn build_qml_modules(
        ...
        for qml_module in &self.qml_modules {
            ...
            let module_init_key = format!("qml_module_{}", module_name_from_uri(&qml_module.uri));
    ...
    fn build_initializers<'a>(
        ...
        let init_call = format!(
            "{declaration}\nstatic const int do_init_{key} = {init_call}",
        ...
        init_lib
            .link_lib_modifier("+whole-archive")
            .compile(&format!("cxxqt-init-lib-{}", key));
    }
    ...
    pub fn build(mut self) {
        ...
        let crate_init_key = format!("crate_{}", crate_name().replace('-', "_"));
        let public_initializer =
