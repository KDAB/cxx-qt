...
pub struct QString {
    ...
    /// Qt6 QString has one member, which contains two pointers and a ssize_t
...
impl QString {
    ...
    /// Replaces every occurrence of the string or character `before` with the string or character `after` and returns a mutable reference to this string.
    ///
    /// If `cs` is [`CaseSensitivity::CaseSensitive`], the search is case-sensitive; otherwise the search is case-insensitive.
    pub fn replace<S: QStringElement>(
        &mut self,
        before: S,
        after: S,
        cs: CaseSensitivity,
