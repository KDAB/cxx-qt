...
#define CXX_QT_QVECTOR_ALIGN_AND_SIZE(name)                                    \
  assert_alignment_and_size(QVector_##name, {                                  \
    ::std::size_t a0;                                                          \
    ::std::size_t a1;                                                          \
    ::std::size_t a2;                                                          \
  });
#else
#define CXX_QT_QVECTOR_ALIGN_AND_SIZE(name)                                    \
  assert_alignment_and_size(QVector_##name, { ::std::size_t a0; });
#endif

#define CXX_QT_QVECTOR_ASSERTS(typeName, name)                                 \
  CXX_QT_QVECTOR_ALIGN_AND_SIZE(name);                                         \
                                                                               \
