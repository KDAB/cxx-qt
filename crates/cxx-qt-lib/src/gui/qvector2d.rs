...
#[cfg(feature = "serde")]
struct QVector2DVisitor;

#[cfg(feature = "serde")]
impl<'de> serde::de::Visitor<'de> for QVector2DVisitor {
    type Value = QVector2D;

    fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
        formatter.write_str("QVector2D")
    }
        ...
        A: serde::de::SeqAccess<'de>,
    {
        ...
        Ok(QVector2D::new(x, y))
    }
}

#[cfg(feature = "serde")]
impl<'de> serde::Deserialize<'de> for QVector2D {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        deserializer.deserialize_tuple(2, QVector2DVisitor)
    }
}

#[cfg(feature = "serde")]
impl serde::Serialize for QVector2D {
...
mod serde_tests {
    ...
    fn test_serde_deserialize() {
        let test_data: QVector2D = serde_json::from_str(r#"[1.0,2.0]"#).unwrap();
        assert_eq!(test_data, QVector2D::new(1.0, 2.0));
    }

    #[test]
    fn test_serde_serialize() {
        let test_data = QVector2D::new(1.0, 2.0);
        let data_string = serde_json::to_string(&test_data).unwrap();
        assert_eq!(data_string, r#"[1.0,2.0]"#);
    }
}
