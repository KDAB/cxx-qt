...
#[cfg(feature = "serde")]
struct QVector4DVisitor;

#[cfg(feature = "serde")]
impl<'de> serde::de::Visitor<'de> for QVector4DVisitor {
    type Value = QVector4D;

    fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
        formatter.write_str("QVector4D")
    }
        ...
        A: serde::de::SeqAccess<'de>,
    {
        ...
        let w = if let Some(value) = seq.next_element::<f32>()? {
            value
        } else {
            return Err(serde::de::Error::invalid_length(
                3,
        ...
        Ok(QVector4D::new(x, y, z, w))
    }
}

#[cfg(feature = "serde")]
impl<'de> serde::Deserialize<'de> for QVector4D {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        deserializer.deserialize_tuple(4, QVector4DVisitor)
    }
}

#[cfg(feature = "serde")]
impl serde::Serialize for QVector4D {
...
mod serde_tests {
    ...
    fn test_serde_deserialize() {
        let test_data: QVector4D = serde_json::from_str(r#"[1.0,2.0,3.0,4.0]"#).unwrap();
        assert_eq!(test_data, QVector4D::new(1.0, 2.0, 3.0, 4.0));
    }

    #[test]
    fn test_serde_serialize() {
        let test_data = QVector4D::new(1.0, 2.0, 3.0, 4.0);
        let data_string = serde_json::to_string(&test_data).unwrap();
        assert_eq!(data_string, r#"[1.0,2.0,3.0,4.0]"#);
    }
}
