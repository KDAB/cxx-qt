...
impl QtBuild {
    ...
    pub fn register_qml_module(
        ...
        let qmlcachegen = if self.qt_installation.version().major >= 6 {
            Some(QtToolQmlCacheGen::new(self.qt_installation.as_ref()))
        } else {
            None
        };
...
"#
        ...
        if self.qt_installation.version().major >= 6 {
            ...
            for file in qml_files {
                ...
                qml_file_qrc_paths.push(result.qml_resource_path);
            }
            ...
            if !qml_files.is_empty() {
                qmlcachegen_file_paths.push(
                    QtToolQmlCacheGen::new(self.qt_installation.as_ref())
                        .compile_loader(qml_cache_args.clone(), &qml_file_qrc_paths),
                );
            }
        }
