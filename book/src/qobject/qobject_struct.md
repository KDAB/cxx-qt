...
- Expose the generated QObject subclass to Rust as [`qobject::MyObject`](./cpp_object.md)
...
These setters and getters assure that the changed signal is called ever time the property is edited.
...
Instead of implementing `Default` by hand, you can also use `#[derive(Default)]`.
